...
- [ ] **CONC-4** Aggregate subprocess count is not globally bounded. A tree-wide live-process cap (filesystem token semaphore) was prototyped and then removed because sync-blocked parents held slots forever; harvest-on-demand was proposed as a fix but is deferred — see `dev/RFC-harvest-on-demand.md` + `dev/SPIKE-A-FINDING.md`. Today: only `CALLSTACK_MAX_FANOUT` and `CALLSTACK_MAX_DEPTH` bound the tree; a wide & deep tree can hold many `claude` subprocesses live at once (~0.5–2 GB RSS each). Revisit when the deferred design becomes viable.
